...
function renderHome(container) {
  ...
  mainProgressText.innerHTML = `
    <div><span class="text-sm font-black text-white">${stats.owned}</span> / ${stats.total} <span class="text-red-400 font-bold ml-1 text-xs">(- ${remaining})</span></div>
  ...
  specialItems.forEach(item => {
    ...
    if (item.prog.owned === item.limit) {
      progBadge.className = 'absolute bottom-2.5 left-1/2 -translate-x-1/2 flex items-center justify-center z-20 pointer-events-none w-full';
      progBadge.innerHTML = `
  ...
  groupsData.forEach(g => {
    ...
    sortedTeams.forEach(team => {
      ...
      if (teamStats.owned === limit) {
        progBadge.className = 'absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center justify-center z-20 pointer-events-none w-full';
        progBadge.innerHTML = `
