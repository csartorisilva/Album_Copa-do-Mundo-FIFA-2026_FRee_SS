...
(function () {
  ...
  window.iniciarLoginGoogle = async function() {
    try {
      const btn = document.querySelector('.google-login-btn');
      if (btn) {
        btn.innerHTML = `<span class="text-xs font-bold text-gray-400 mx-auto">🔄 Conectando ao Google...</span>`;
        btn.disabled = true;
      }
      ...
      if (btn) {
        ...
        btn.innerHTML = `
          ...
          <span class="font-bold text-sm text-gray-200 group-hover:text-white flex-1 text-left">Google Account</span>
          <span class="text-gray-500 group-hover:text-copaYellow transition">➔</span>
        `;
        btn.disabled = false;
      }
    }
